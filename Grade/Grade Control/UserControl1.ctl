...
Dim TabID() As String
Dim TabC() As String
Dim CatsID() As String
Dim CatsC() As String
Dim CatsT() As String
Dim CatsD() As Boolean
Dim TopBID() As String
Dim TopBC() As String

Dim TopBuID() As String
Dim TopBuS() As String
Dim TopBuC() As String
Dim TopBuI() As Picture
Dim TopBuT() As String
Dim TopBuG() As Boolean
...
TotalCatsT = 0
If TotalCats > 0 Then
    ReDim CatsIDT(0 To TotalCats - 1)
    ReDim CatsCT(0 To TotalCats - 1)
    ReDim CatsTT(0 To TotalCats - 1)
    ReDim CatsDT(0 To TotalCats - 1)
...
Public Function AddTab(zID As String, zCaption As String) As Boolean
    ...
    ReDim Preserve TabID(0 To TotalTabs - 1)
    TabID(TotalTabs - 1) = zID
    ReDim Preserve TabC(0 To TotalTabs - 1)
...
Public Function AddCat(zID As String, zTab As String, zCaption As String, zDlgButton As Boolean) As Boolean
    ...
    ReDim Preserve CatsID(0 To TotalCats - 1)
    CatsID(TotalCats - 1) = zID
    ReDim Preserve CatsT(0 To TotalCats - 1)
    CatsT(TotalCats - 1) = zTab
    ReDim Preserve CatsC(0 To TotalCats - 1)
    CatsC(TotalCats - 1) = zCaption
    ReDim Preserve CatsD(0 To TotalCats - 1)
...
Public Function AddTopButton(zID As String, zCaption As String, zPicture As Picture) As Boolean
    ...
    ReDim Preserve TopBID(0 To TotalTopButton - 1)
    TopBID(TotalTopButton - 1) = zID
    ReDim Preserve TopBC(0 To TotalTopButton - 1)
...
Private Sub RibbonTopCustom_over_Click()
    ...
    '直接判断m_bFold变量的值来确定控件的高度。
    'm_bFold=Ture 时,即收起选项卡部分
    'm_bFold=False 时,即复原选项卡部分
    m_bFold = Not (m_bFold)
    UserControl.Height = IIf(m_bFold, 730, Barra2.Height)
End Sub
...
Public Function AddButton(zID As String, zSubCat As String, zCaption As String, zPicture As Picture, Optional zMore As Boolean = False, Optional zToolTip As String) As Boolean
    TotalButton = TotalButton + 1
        ReDim Preserve TopBuID(0 To TotalButton - 1)
        TopBuID(TotalButton - 1) = zID
        ReDim Preserve TopBuS(0 To TotalButton - 1)
        TopBuS(TotalButton - 1) = zSubCat
        ReDim Preserve TopBuC(0 To TotalButton - 1)
        TopBuC(TotalButton - 1) = zCaption
        ReDim Preserve TopBuT(0 To TotalButton - 1)
        If zToolTip = "" Or zToolTip = Null Then
            TopBuT(TotalButton - 1) = zCaption
        Else
            TopBuT(TotalButton - 1) = zToolTip
        End If
        ReDim Preserve TopBuI(0 To TotalButton - 1)
        Set TopBuI(TotalButton - 1) = zPicture
        ReDim Preserve TopBuG(0 To TotalButton - 1)
...
TotalSize = 0
TotalButtonT = 0
ReDim TopBuIDT(0 To TotalButton - 1)
ReDim TopBuST(0 To TotalButton - 1)
ReDim TopBuCT(0 To TotalButton - 1)
ReDim TopBuIT(0 To TotalButton - 1)
ReDim TopBuTT(0 To TotalButton - 1)
ReDim TopBuGT(0 To TotalButton - 1)
...
Private Function Find(name As String) As Integer
  For i = 0 To TotalButton - 1
    If TopBuC(i) = name Then
       Find = i
       Exit Function
    End If
  Next i
...
Public Function DelButton(zCaption As String)
  On Error Resume Next
  Dim Inx As Integer, i As Integer
  Inx = Find(zCaption)
  If Inx = 9999 Then Exit Function
    TopBuID(Inx) = ""
    TopBuS(Inx) = ""
    TopBuC(Inx) = ""
    Set TopBuI(Inx) = Nothing
    TopBuT(Inx) = ""
  TotalButton = TotalButton - 1
    For i = Inx To TotalButton - 1
      TopBuID(i) = TopBuID(i + 1)
      TopBuS(i) = TopBuS(i + 1)
      TopBuC(i) = TopBuC(i + 1)
      Set TopBuI(i) = TopBuI(i + 1)
      TopBuT(i) = TopBuT(i + 1)
      TopBuG(i) = TopBuG(i + 1)
    Next i
    ReDim Preserve TopBuID(0 To TotalButton - 1)
